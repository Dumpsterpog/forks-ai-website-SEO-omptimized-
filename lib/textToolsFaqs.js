...
export const LOREM_IPSUM_FAQS = [
  {
    q: "What is lorem ipsum?",
    a: "It is placeholder text: a scrambled, partly invented version of a passage from Cicero's De Finibus Bonorum et Malorum, written in 45 BC. Typesetters have used it since the sixteenth century because it looks like ordinary prose without meaning anything, so nobody reads it instead of looking at the layout.",
  },
  {
    q: "Why not just type the same sentence over and over?",
    a: "Repeated text has a visual rhythm real copy does not. Line lengths line up, hyphenation never happens and the block looks tidier than the finished page will. Placeholder text with varied word and sentence lengths shows you where a design breaks before the real words arrive.",
  },
  {
    q: "Does it generate the same text every time?",
    a: "Only if you leave the settings alone. The generator is seeded, so a given set of options always produces the same passage, and the generate again button changes the seed. That makes a block you liked reproducible instead of gone on the next click.",
  },
  {
    q: "What is the difference between the plain text and HTML output?",
    a: "Plain text separates paragraphs with a blank line, which is what a document or a design tool expects on paste. The HTML output wraps each paragraph in a p element and nothing else, with no class names and no wrapper, so it drops into existing markup without bringing styling with it.",
  },
  {
    q: "How many words are in a paragraph?",
    a: "Each paragraph here is three to six sentences and each sentence is six to sixteen words, so a paragraph runs roughly 20 to 90 words. The word and character counts for the passage you actually generated are shown above the output. If you need an exact length, switch the unit to words and type the number.",
  },
  {
    q: "Is anything sent to a server?",
    a: "No. The text is generated in your browser by a few lines of JavaScript, so there is no request to make and nothing to log. The tool works with the network off.",
  },
];
